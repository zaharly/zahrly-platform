  ...
  return (
    <div className="flex flex-col gap-density-xl">
      ...
      <div className="grid grid-cols-2 gap-density-md md:grid-cols-4 xl:grid-cols-4">
        ...
        <MetricCard label="Historical Bootstrap" value={`${BOOTSTRAP_CAMPAIGN.overallCompletenessPct}%`} icon={History} tone="info" sublabel="2020 – 2026 campaign, background/production-safe" />
      ...
      <div className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm"><div className="mb-density-md flex items-center justify-between"><div><h2 className="text-base font-semibold text-foreground">Historical Bootstrap Progress</h2><p className="text-sm text-muted-foreground">Multi-month acquisition campaign — yields to production, never reprocesses the 7-day horizon.</p></div><Link to="/bootstrap" className="inline-flex items-center gap-1 text-sm font-medium text-foreground hover:underline">Open Bootstrap Control Center <ArrowRight className="h-3.5 w-3.5" /></Link></div><div className="grid grid-cols-2 gap-density-md sm:grid-cols-4 lg:grid-cols-7">{BOOTSTRAP_SEASONS.map((season) => <div key={season.season} className="flex flex-col gap-2 rounded-md border border-border p-density-sm"><div className="flex items-center justify-between"><span className="text-sm font-semibold text-foreground">{season.season}</span><StatusBadge status={season.status} dense /></div><ProgressBar value={season.coreCompletenessPct} showValue={false} size="sm" tone={season.coreCompletenessPct >= 95 ? 'success' : season.coreCompletenessPct >= 50 ? 'warning' : 'critical'} /><span className="text-xs text-muted-foreground">{season.coreCompletenessPct.toFixed(0)}% core · {season.fixtures.toLocaleString()} fixtures</span></div>)}</div></div>
