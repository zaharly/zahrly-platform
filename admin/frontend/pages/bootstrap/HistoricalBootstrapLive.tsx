...
export default function HistoricalBootstrapLive() {
  ...
  return (
    <div className="flex flex-col gap-density-xl">
      <PageHeader title="Historical Bootstrap" description="Choose one season. Zahrly keeps the enabled catalog scope stable, filters availability for that season, builds the complete historical dataset plan, then creates dataset jobs." tag={<StatusBadge status={loading ? 'LOADING' : seasonRow?.gate_state ?? (availableLeagues.length ? 'READY' : 'BLOCKED')} />} actions={<Button variant="outline" onClick={() => void load(selectedSeason)} disabled={loading}><RefreshCw className="h-4 w-4" /> Refresh</Button>} />
        ...
        <div className="grid gap-density-lg lg:grid-cols-[280px,1fr]">
          ...
          <div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-4">
              ...
              <Metric label="Campaign" value={campaign?.status ?? scope?.campaign?.status ?? 'Not started'} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold">Campaign & provider guardrails</h2><p className="mt-1 text-sm text-muted-foreground">Budget is controlled automatically from the provider quota. These values are read-only operational information.</p></div><StatusBadge status="AUTO-QUOTA" dense /></div>
        <div className="mt-4 grid gap-4 md:grid-cols-4">
          <Metric label="Daily provider budget" value={quota.daily_budget == null ? '—' : String(quota.daily_budget)} />
          <Metric label="Quota used" value={quota.quota_used == null ? '—' : String(quota.quota_used)} />
          <Metric label="Backfill remaining" value={quota.backfill_budget == null ? '—' : String(quota.backfill_budget)} />
          <Metric label="Campaign requests used" value={String(scope?.campaign?.requests_used ?? 0)} />
        </div>
        <div className="mt-4 grid gap-4 md:grid-cols-3 text-xs text-muted-foreground">
          <div><span className="uppercase">Planned start</span><div className="mt-1 text-sm font-medium text-foreground">{fmtDate(scope?.campaign?.planned_start_at)}</div></div>
          <div><span className="uppercase">Minimum target end</span><div className="mt-1 text-sm font-medium text-foreground">{fmtDate(scope?.campaign?.minimum_target_end_at)}</div></div>
          <div><span className="uppercase">Reserve policy</span><div className="mt-1 text-sm font-medium text-foreground">{quota.reserve_policy_version ?? '—'}</div></div>
        </div>
      </section>
      ...
      <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm">
        <div className="flex flex-col gap-density-lg lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex gap-2"><Button onClick={() => void prepareOnly()} disabled={preparing || starting || loading || availableLeagues.length === 0} variant="outline"><Database className="h-4 w-4" /> {preparing ? 'Preparing…' : 'Prepare jobs'}</Button><Button onClick={() => void startBootstrap()} disabled={starting || preparing || loading || availableLeagues.length === 0}><Play className="h-4 w-4" /> {starting ? 'Starting…' : 'Start Historical Bootstrap'}</Button></div>
        </div>

        {availableLeagues.length === 0 ? <Notice title="No enabled scope for this season" text="No enabled Country + League combination is available for this season in the provider catalog." /> : <div className="mt-5 max-h-[420px] overflow-auto rounded-md border border-border"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs uppercase text-muted-foreground"><th className="px-3 py-2">Country</th><th className="px-3 py-2">League</th><th className="px-3 py-2">Provider season</th><th className="px-3 py-2">Ingestion</th></tr></thead><tbody>{availableLeagues.map((league) => <tr key={league.id} className="border-b last:border-0"><td className="px-3 py-3">{enabledCountries.find((country) => country.id === league.country_id)?.name ?? '—'}</td><td className="px-3 py-3 font-medium">{league.name}</td><td className="px-3 py-3">{displaySeason(selectedSeason)}</td><td className="px-3 py-3"><StatusBadge status="ENABLED" dense /></td></tr>)}</tbody></table></div>}
      </section>

      <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm"><div className="flex items-center justify-between"><div><h2 className="text-base font-semibold">Season progress</h2><p className="mt-1 text-sm text-muted-foreground">Completeness is seasonal. A complete historical season does not force another season to be processed.</p></div><div className="text-xs text-muted-foreground">{seasonRow ? `${seasonRow.backfill_succeeded}/${seasonRow.backfill_jobs} jobs succeeded` : 'Not started'}</div></div><div className="mt-4"><ProgressBar value={Math.max(0, Math.min(100, Number(seasonRow?.backfill_progress ?? 0)))} /></div><div className="mt-2 grid grid-cols-2 gap-3 text-xs text-muted-foreground md:grid-cols-4"><Metric label="Queued" value={String(seasonRow?.backfill_jobs ?? 0)} /><Metric label="Succeeded" value={String(seasonRow?.backfill_succeeded ?? 0)} /><Metric label="Failed" value={String(seasonRow?.backfill_failed ?? 0)} /><Metric label="Archive completeness" value={seasonRow ? `${(Number(seasonRow.archive_completeness ?? 0) * 100).toFixed(1)}%` : '—'} /></div></section>
    </div>
  )
}
...
function DatasetSection({ title, datasets }: { title: string; datasets: HistoricalBootstrapDatasetPlan[] }) {
  ...
  return <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm"><div><h2 className="text-base font-semibold">{title}</h2><p className="mt-1 text-sm text-muted-foreground">Green means the dataset is fetchable for the selected season scope. Partial means only some enabled leagues expose the provider capability.</p></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{datasets.map((dataset) => { const state = datasetStatus(dataset); const isGreen = state === 'AVAILABLE'; return <div key={dataset.dataset_key} className={`rounded-md border p-3 text-sm ${isGreen ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-border bg-card'}`}><div className="flex items-center gap-2"><CheckCircle2 className={`h-4 w-4 ${isGreen ? 'text-emerald-600' : 'text-muted-foreground'}`} /><span className={isGreen ? 'font-medium text-emerald-700 dark:text-emerald-400' : 'font-medium'}>{dataset.label}</span></div><div className="mt-2 flex items-center justify-between text-xs text-muted-foreground"><span>{dataset.endpoint}</span><span>{dataset.available_count}/{dataset.scope_count}</span></div></div> })}</div></section>
}
