...
import { fetchHistoricalBootstrapScope, type HistoricalBootstrapScope } from '../../integrations/historicalBootstrapScope'
...
export default function HistoricalBootstrapLive() {
  ...
  const seasonRow = useMemo(
    () => dedupeSeasons(snapshot?.seasons ?? []).find((row) => row.season === selectedSeason) ?? null,
    [snapshot?.seasons, selectedSeason],
  )
  ...
  const enabledLeagues = scope?.competitions ?? []
  ...
  async function startBootstrap() {
    ...
    try {
      ...
      toast.success(`Historical ${displaySeason(selectedSeason)} started`, {
        description: `${result.jobs_total} jobs prepared from the enabled season scope.`,
      })
  ...
  async function prepareOnly() {
    ...
    try {
      const result = await prepareHistoricalSeason(selectedSeason, 100)
      toast.success(`Season ${displaySeason(selectedSeason)} prepared`, {
        description: `${result.jobs_total} backfill jobs in scope.`,
      })
      await load(selectedSeason)
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-density-xl">
      <PageHeader
        ...
        description="Choose one season. Zahrly resolves enabled Countries + Leagues from the provider catalog, builds the dataset plan, then creates historical jobs."
        tag={<StatusBadge status={loading ? 'LOADING' : seasonRow?.gate_state ?? (enabledLeagues.length ? 'READY' : 'BLOCKED')} />}
      ...
      <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm">
        <div className="grid gap-density-lg lg:grid-cols-[280px,1fr]">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">No Country or League is selected manually. The selected season is resolved against the synchronized provider catalog and ingestion toggles.</p>
      ...
      <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm">
        <div className="flex items-center justify-between gap-3">
          <div><h2 className="text-base font-semibold">Dataset plan</h2><p className="mt-1 text-sm text-muted-foreground">The historical contract collects these datasets for the resolved season scope. Archive completeness is measured per season.</p></div>
          <StatusBadge status="PRE-S3" dense />
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {DATASETS.map((dataset) => <div key={dataset} className="rounded-md border border-border p-3 text-sm"><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4" />{dataset}</div></div>)}
        </div>
      </section>
      ...
      <section className="rounded-lg border border-border bg-card p-density-lg shadow-retool-sm">
        <div className="flex flex-col gap-density-lg lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2"><h2 className="text-base font-semibold">Review {displaySeason(selectedSeason)}</h2><StatusBadge status={seasonRow?.gate_state ?? (enabledLeagues.length ? 'READY' : 'BLOCKED')} dense /></div>
            <p className="mt-1 text-sm text-muted-foreground">The final scope is Country enabled AND League enabled AND season available in the provider catalog.</p>
          </div>
          <div className="flex gap-2">
            <Button onClick={() => void prepareOnly()} disabled={preparing || starting || loading || enabledLeagues.length === 0} variant="outline"><Database className="h-4 w-4" /> {preparing ? 'Preparing…' : 'Prepare jobs'}</Button>
            <Button onClick={() => void startBootstrap()} disabled={starting || preparing || loading || enabledLeagues.length === 0}><Play className="h-4 w-4" /> {starting ? 'Starting…' : 'Start Historical Bootstrap'}</Button>
          </div>
        </div>

        {enabledLeagues.length === 0 ? (
          <Notice title="No enabled scope for this season" text="There are no enabled Country + League combinations with this season in the provider catalog." />
        ) : (
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div><div className="text-xs uppercase text-muted-foreground">{label}</div><div className="mt-1 font-semibold">{value}</div></div>
}
