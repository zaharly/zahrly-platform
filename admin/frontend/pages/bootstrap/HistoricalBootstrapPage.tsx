...
import { fetchHistoricalBootstrapSnapshot, startHistoricalCampaign, type HistoricalBootstrapSnapshot, type HistoricalSeasonProgress } from '../../integrations/archiveLive'
...
async function loadCampaignData(season: number) {
  const [nextSnapshot, nextScope] = await Promise.all([
    fetchHistoricalBootstrapSnapshot(),
    fetchHistoricalBootstrapScope(season),
  ])
  return { nextSnapshot, nextScope }
}
...
export default function HistoricalBootstrapPage() {
  ...
  async function refreshCampaign(season: number) {
    setDetailLoading(true)
    try {
      const { nextSnapshot, nextScope } = await loadCampaignData(season)
      setSnapshot(nextSnapshot)
      setScope(nextScope)
      toast.success('Campaign data refreshed')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to refresh campaign data')
    } finally {
      setDetailLoading(false)
    }
  }
  ...
  async function startBootstrap() {
    ...
    try {
      ...
      toast.success(`Historical ${displaySeason(detailSeason)} started`, { description: 'The queue will follow the campaign start/target window and provider quota limits.' })
      const { nextSnapshot, nextScope } = await loadCampaignData(detailSeason)
      setSnapshot(nextSnapshot)
      setScope(nextScope)
    } catch (error) {
  ...
  if (detailSeason != null) return <CampaignDetail season={detailSeason} row={detailRow} scope={scope} loading={detailLoading} starting={starting} datasetOpen={datasetOpen} onToggleDataset={() => setDatasetOpen((v) => !v)} onBack={closeCampaign} onRefresh={() => void refreshCampaign(detailSeason)} onStart={() => void startBootstrap()} />

  return <><div className="mx-auto flex w-full max-w-[1200px] flex-col gap-5 pb-8"><PageHeader title="Historical Bootstrap" description="Manage historical data campaigns and their execution." tag={<span className="inline-flex items-center rounded-full border border-border bg-muted px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">{loading ? 'Loading…' : `${seasons.length} campaigns`}</span>} actions={<Button onClick={() => setCreateOpen(true)}><Database className="h-4 w-4" /> Create Campaign</Button>} /><section className="rounded-xl border border-border bg-card shadow-retool-sm"><div className="flex items-center justify-between border-b border-border px-5 py-4"><div><h2 className="text-sm font-semibold">Campaigns</h2><p className="mt-1 text-xs text-muted-foreground">Only campaign-level information is shown here.</p></div><Button variant="outline" size="sm" onClick={() => void loadSnapshot()} disabled={loading}><RefreshCw className="h-3.5 w-3.5" /> Refresh</Button></div>{loading ? <div className="p-8 text-center text-sm text-muted-foreground">Loading campaigns…</div> : seasons.length === 0 ? <div className="p-10 text-center text-sm text-muted-foreground">No campaigns yet.</div> : <div className="divide-y divide-border">{seasons.map((row) => <CampaignRow key={row.season} row={row} onOpen={() => void openCampaign(row.season)} />)}</div>}</section></div>{createOpen && <HistoricalCampaignCreateDialog onClose={() => setCreateOpen(false)} onCreated={created} />}</>
}
...
function CampaignDetail({ season, row, scope, loading, starting, datasetOpen, onToggleDataset, onBack, onRefresh, onStart }: { season: number; row: HistoricalSeasonProgress | null; scope: HistoricalBootstrapScope | null; loading: boolean; starting: boolean; datasetOpen: boolean; onToggleDataset: () => void; onBack: () => void; onRefresh: () => void; onStart: () => void }) {
  ...
  const datasets = scope?.dataset_plan ?? []
  const progress = progressValue(row)
  const archive = row?.archive_completeness == null ? '—' : `${(Number(row.archive_completeness) * 100).toFixed(1)}%`
  const processable = datasets.filter((d) => datasetStatus(d) !== 'UNAVAILABLE')
  const blocked = datasets.filter((d) => datasetStatus(d) === 'UNAVAILABLE')
  const quota = scope?.quota ?? {}
  return <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-5 pb-8">
    <div className="flex items-center justify-between"><Button variant="ghost" size="sm" onClick={onBack}><ArrowLeft className="h-4 w-4" /> Back to Campaigns</Button><Button variant="outline" size="sm" onClick={onRefresh} disabled={loading}><RefreshCw className="h-3.5 w-3.5" /> Refresh</Button></div>
    <PageHeader title={`Run ${displaySeason(season)}`} description="Historical Bootstrap campaign" tag={<StatusBadge status={loading ? 'LOADING' : (row ? seasonStatus(row) : 'READY')} />} actions={<Button onClick={onStart} disabled={starting || loading || !scope?.available_league_count}><Play className="h-4 w-4" /> {starting ? 'Starting…' : 'Start Bootstrap'}</Button>} />
...
function datasetStatusMeta(state: ReturnType<typeof datasetStatus>) {
  if (state === 'AVAILABLE') return { Icon: CheckCircle2, className: 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-300' }
  if (state === 'PARTIAL') return { Icon: AlertTriangle, className: 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300' }
  return { Icon: XCircle, className: 'border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900/40 dark:bg-rose-950/20 dark:text-rose-300' }
}

function DatasetTable({ datasets }: { datasets: HistoricalBootstrapDatasetPlan[] }) {
  return <div className="overflow-hidden rounded-md border border-border">
    <div className="grid grid-cols-[minmax(190px,1.6fr),82px,78px,92px,92px,92px,105px] border-b bg-muted/30 px-4 py-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
      <div>Dataset</div><div>Type</div><div>Required</div><div>Total</div><div>Processed</div><div>Remaining</div><div>Status</div>
    </div>
    <div className="max-h-[420px] overflow-auto divide-y divide-border">
      {datasets.map((d) => {
        const state = datasetStatus(d)
        const { Icon, className } = datasetStatusMeta(state)
        const processedRatio = d.total_count > 0 ? (d.processed_count / d.total_count) * 100 : 0
        return <div key={d.dataset_key} className="grid grid-cols-[minmax(190px,1.6fr),82px,78px,92px,92px,92px,105px] items-center gap-2 px-4 py-2.5 text-xs hover:bg-muted/20">
          ...
          <div>{numberValue(d.total_count)}</div>
          <div><div className="font-semibold">{numberValue(d.processed_count)}</div><div className="text-[10px] text-muted-foreground">{processedRatio.toFixed(0)}%</div></div>
          <div className="text-muted-foreground">{numberValue(d.remaining_count)}</div>
          <div><span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${className}`}><Icon className="h-3 w-3" />{state}</span></div>
        </div>
      })}
    </div>
  </div>
