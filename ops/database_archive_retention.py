...
def archive_query(conn, client, query: str, key: str) -> tuple[int, str, int]:
    rows = 0
    ...
    client.upload_file(str(path), BUCKET, key, ExtraArgs={"ContentType": "application/x-ndjson", "ContentEncoding": "gzip", "Metadata": {"sha256": digest, "row_count": str(rows), "archived_at": datetime.now(timezone.utc).isoformat()}})
...
def main() -> None:
    ...
    with db() as conn:
        ...
        oos_candidates = fetch_ids(
            ...
            WHERE pr.status='SUCCEEDED'
              ...
              AND pr.id NOT IN (
                ...
                WHERE status='SUCCEEDED' ORDER BY finished_at DESC NULLS LAST LIMIT %s
              )
        ...
        if oos_candidates:
            ...
            q = f"SELECT row_to_json(x)::text || E'\\n' FROM (SELECT * FROM internal.prediction_oos_benchmark WHERE training_run_id IN ({lit}) ORDER BY training_run_id, fold_no, played_at, fixture_id) x"
            rows, digest, size = archive_query(conn, client, q, key)
            conn.execute(f"DELETE FROM internal.prediction_oos_benchmark WHERE training_run_id IN ({lit})")
            conn.commit()
        ...
        rating_candidates = fetch_ids(
            ...
            WHERE pr.status='SUCCEEDED'
              ...
              AND mv.id NOT IN (
                ...
                ORDER BY mv2.created_at DESC LIMIT %s
              )
        ...
        if rating_candidates:
            ...
            q = f"SELECT row_to_json(x)::text || E'\\n' FROM (SELECT * FROM internal.prediction_rating_checkpoints WHERE model_version_id IN ({lit}) ORDER BY model_version_id, checkpoint_scope, as_of_time, team_id) x"
            rows, digest, size = archive_query(conn, client, q, key)
            conn.execute(f"DELETE FROM internal.prediction_rating_checkpoints WHERE model_version_id IN ({lit})")
            conn.commit()
        ...
        if cron_count:
            ...
            q = "SELECT row_to_json(x)::text || E'\\n' FROM (SELECT * FROM cron.job_run_details WHERE start_time < now() - make_interval(days => %s) ORDER BY start_time, runid) x" % CRON_RETENTION_DAYS
            rows, digest, size = archive_query(conn, client, q, key)
            conn.execute("DELETE FROM cron.job_run_details WHERE start_time < now() - make_interval(days => %s)", (CRON_RETENTION_DAYS,))
            conn.commit()
            results.append({"table": "cron.job_run_details", "rows": rows, "s3_key": key, "sha256": digest, "bytes": size})

    # Regular vacuum is non-blocking enough for this maintenance task and makes reclaimed pages reusable.
    with db() as conn:
