...
-- Some deployed databases already have evidence_seq as a GENERATED ALWAYS AS IDENTITY
-- column, while the runtime appends a deterministic per-baseline sequence. Keep the
-- identity sequence as a safe default, but allow the runtime to provide an explicit
-- per-baseline sequence so the existing append-only contract remains compatible.
...
begin
  ...
  elsif identity_mode = 'identity' then
    alter table public.prediction_evidence_updates
      alter column evidence_seq set generated by default;
  end if;
end
$$;
