...
def export_rows(conn, table: str, where_sql: str, params: tuple, path: Path) -> int:
    ...
    with gzip.open(path, "wb", compresslevel=6) as out:
        with conn.cursor() as cur:
            cur.execute(f"select * from internal.{table} where {where_sql} order by id")
            while True:
...
def register(conn, artifact_type: str, training_run_id: str, model_version_id: str, uri: str, sha: str, size: int, rows: int):
    conn.execute(
        ...
        (
            ...
            size,
            rows,
            json.dumps({"storage": "s3", "verified": True}),
        ),
    )
...
def main() -> None:
    ...
    with db_connect() as conn:
        ...
        with tempfile.TemporaryDirectory(prefix="zahrly-prediction-archive-") as tmp:
            for artifact_type, table, prefix in ARTIFACTS:
                if artifact_type == "OOS_BENCHMARK":
                    where_sql = "training_run_id=%s"
                    params = (run["training_run_id"],)
                else:
                    where_sql = "model_version_id=%s"
                    params = (run["model_version_id"],)
                ...
                if artifact_type == "OOS_BENCHMARK":
                    deleted = conn.execute(
                        "delete from internal.prediction_oos_benchmark where training_run_id=%s returning id",
                        (run["training_run_id"],),
                    ).rowcount
                else:
                    deleted = conn.execute(
                        "delete from internal.prediction_rating_checkpoints where model_version_id=%s returning id",
        ...
        conn.execute(
            """
            update internal.prediction_training_runs
