        ...
        with conn.cursor() as cur:
            ...
            while True:
                ...
                if not rows:
                    break
                for row in rows:
                    out.write(json.dumps(dict(row), ensure_ascii=False, separators=(",", ":"), default=str).encode("utf-8") + b"\n")
                    count += 1
    return count


def upload_verified(s3, bucket: str, key: str, path: Path) -> tuple[str, int]:
    ...
    if int(head.get("ContentLength", -1)) != size:
        raise RuntimeError(f"s3_size_verification_failed:{key}")
    return sha, size
...
def register(conn, artifact_type: str, training_run_id: str, model_version_id: str, uri: str, sha: str, size: int, rows: int):
    ...
    """, (
        ...
        json.dumps({"storage": "s3", "verified": True, "format": "ndjson-gzip", "source_table": artifact_type}),
    ))
...
def main():
    ...
    with db_connect() as conn:
        if requested_run:
            run = conn.execute("""
                select tr.id::text as training_run_id, tr.model_version_id::text as model_version_id, mv.version
                from internal.prediction_training_runs tr join public.model_versions mv on mv.id=tr.model_version_id
                where tr.status='SUCCEEDED' and tr.id=%s
            """, (requested_run,)).fetchone()
        else:
            run = conn.execute("""
                select tr.id::text as training_run_id, tr.model_version_id::text as model_version_id, mv.version
                from internal.prediction_training_runs tr join public.model_versions mv on mv.id=tr.model_version_id
                where tr.status='SUCCEEDED' order by tr.started_at desc limit 1
            """).fetchone()
        if not run:
        ...
        conn.execute("update internal.prediction_training_runs set metrics=coalesce(metrics,'{}'::jsonb)||%s::jsonb where id=%s",
