...
def _s3_client():
    kwargs: dict[str, Any] = {'service_name':'s3','region_name':_env('S3_REGION'),'aws_access_key_id':_env('S3_ACCESS_KEY_ID'),'aws_secret_access_key':_env('S3_SECRET_ACCESS_KEY'),'config':Config(retries={'max_attempts':5,'mode':'standard'})}
...
def _season_from_uri(uri: str) -> int | None:
    import re
...
def _number(value: Any) -> int | None:
    ...
    if isinstance(value,str):
        try:
            x=float(value.strip()); return int(x) if x.is_integer() else None
...
def load_settled_matches(conn, as_of: datetime | None=None) -> list[Match]:
    ...
    team_map=fetch_team_identity_map(conn); client=_s3_client(); cutoff=_as_datetime(as_of or datetime.now(timezone.utc))
    by_id: dict[str,Match]={}
    for manifest in manifests:
        bucket,key=_parse_uri(manifest.object_uri); season=_season_from_uri(manifest.object_uri)
        ...
        for row in _walk(doc):
            fixture=row.get('fixture') or {}; teams=row.get('teams') or {}; goals=row.get('goals') or {}
            fid=fixture.get('id'); date=fixture.get('date'); home=(teams.get('home') or {}).get('id'); away=(teams.get('away') or {}).get('id'); hg=_number(goals.get('home')); ag=_number(goals.get('away'))
            ...
            home_team=_historical_team_key(home,team_map.get(str(home))); away_team=_historical_team_key(away,team_map.get(str(away)))
            if not home_team or not away_team: continue
            candidate=Match(str(fid),played,home_team,away_team,hg,ag,season)
            existing=by_id.get(str(fid))
            if existing is None:
                by_id[str(fid)]=candidate
            elif existing != candidate:
                # Same fixture may be present in multiple archived manifests. Keep
                # one canonical observation and fail only when the actual match
                # payload disagrees, rather than silently changing its season.
                if (existing.played_at,existing.home_team_id,existing.away_team_id,existing.home_goals,existing.away_goals) != (candidate.played_at,candidate.home_team_id,candidate.away_team_id,candidate.home_goals,candidate.away_goals):
