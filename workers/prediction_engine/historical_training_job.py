...
def build_cutoffs(matches):
 ordered=sorted(matches,key=lambda m:m.played_at);seasons=sorted({m.season for m in ordered if m.season is not None})
 if len(seasons)>=2:return [min(m.played_at for m in ordered if m.season==season) for season in seasons[1:]]
...
def main():
 ...
 with db_connect() as conn:
  ...
  cutoffs=build_cutoffs(matches);folds=build_walk_forward_folds(matches,cutoffs);preview=[{'cutoff':fc.isoformat(),'train':len(train),'test':len(test),'train_seasons':sorted({m.season for m in train if m.season is not None}),'test_seasons':sorted({m.season for m in test if m.season is not None})} for train,test,fc in folds]
  ...
  try:
   ...
   summaries=[];total=tb=tl=tr=0.;years=[];usable=[(train,test,fc) for train,test,fc in folds if train and test];all_oos=[m for _,test,_ in usable for m in test];all_features=build_feature_index_for_matches(conn,all_oos,min(fc for _,_,fc in usable)) if usable else {}
   ...
   ep=EloPolicy();gp=GlickoPolicy();dp=DixonColesPolicy();elo=_fit_elo(matches,cutoff,ep);g=_fit_glicko(matches,cutoff,gp);rate,attack,defense=_fit_dc(matches,cutoff,dp);metrics={'settled_matches':len(matches),'folds':len(summaries),'complete_oos_seasons':len(set(years)),'predictions':int(total),'brier_1x2_mean':tb/total,'log_loss_1x2_mean':tl/total,'rps_1x2_mean':tr/total,'folds_detail':summaries,'feature_layer':{'enabled':True,'datasets':'historical_archive','no_same_fixture_post_match_leakage':True},'validation_status':'ELIGIBLE' if total>=MIN_OOS_SAMPLES and len(set(years))>=MIN_COMPLETE_SEASONS else 'INSUFFICIENT_HISTORY','validation_eligible':total>=MIN_OOS_SAMPLES and len(set(years))>=MIN_COMPLETE_SEASONS,'promotion_blocked':True}
