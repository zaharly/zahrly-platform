...
def _fold_metrics(test, predictions):
    n=len(predictions)
    if not n: return {'brier_1x2':None,'log_loss_1x2':None,'rps_1x2':None,'ece_1x2':None}
    brier=logloss=rps=0.0
    bins=[{'n':0,'confidence':0.0,'accuracy':0.0} for _ in range(10)]
    for match,pred in zip(test,predictions):
        probs=(max(1e-15,pred.p_home),max(1e-15,pred.p_draw),max(1e-15,pred.p_away))
        ...
        brier += sum((p-y)**2 for p,y in zip(probs,ys)); logloss -= log(probs[actual]); rps += ((probs[0]-ys[0])**2 + (probs[0]+probs[1]-ys[0]-ys[1])**2)/2.0
        confidence=max(probs); predicted=max(range(3),key=lambda i:probs[i]); idx=min(9,int(confidence*10.0))
...
def main():
    ...
    with db_connect() as conn:
        ...
        if not cutoffs: raise RuntimeError('prediction_training_gate_failed:no_valid_cutoffs')
        ...
        try:
            ...
            for fold_no,(train,test,fold_cutoff) in enumerate(folds,1):
                if not train or not test:
                    continue
                predictions=run_fold(train,test,fold_cutoff); score=_fold_metrics(test,predictions); total_predictions+=len(predictions)
                total_brier += score['brier_1x2']*len(predictions); total_logloss += score['log_loss_1x2']*len(predictions); total_rps += score['rps_1x2']*len(predictions); fold_years.append(fold_cutoff.year)
            ...
            if total_predictions==0: raise RuntimeError('prediction_training_gate_failed:no_oos_predictions')
