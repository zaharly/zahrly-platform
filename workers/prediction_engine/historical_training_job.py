...
def build_cutoffs(matches):
 ordered=sorted(matches,key=lambda m:m.played_at);seasons=sorted({str(m.season) for m in ordered if m.season is not None},key=lambda s:int(s.split('/',1)[0]) if s.split('/',1)[0].isdigit() else 10**9)
 if len(seasons)>=2:return [min(m.played_at for m in ordered if str(m.season)==season) for season in seasons[1:]]
...
def main():
 ...
 with db_connect() as conn:
  ...
  try:
   ...
   ep=EloPolicy();gp=GlickoPolicy();dp=DixonColesPolicy();elo=_fit_elo(matches,cutoff,ep);g=_fit_glicko(matches,cutoff,gp);rate,attack,defense=_fit_dc(matches,cutoff,dp);metrics={'settled_matches':len(matches),'folds':len(summaries),'complete_oos_seasons':len(oos_seasons),'oos_seasons':sorted(oos_seasons,key=lambda s:int(s.split('/',1)[0]) if s.split('/',1)[0].isdigit() else 10**9),'predictions':int(total),'brier_1x2_mean':tb/total,'log_loss_1x2_mean':tl/total,'rps_1x2_mean':tr/total,'folds_detail':summaries,'feature_layer':{'enabled':True,'datasets':'historical_archive','no_same_fixture_post_match_leakage':True},'validation_status':'ELIGIBLE' if total>=MIN_OOS_SAMPLES and len(oos_seasons)>=MIN_COMPLETE_SEASONS else 'INSUFFICIENT_HISTORY','validation_eligible':total>=MIN_OOS_SAMPLES and len(oos_seasons)>=MIN_COMPLETE_SEASONS,'promotion_blocked':True}
