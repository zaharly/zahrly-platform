...
def build_cutoffs(start,end):
 s=start.astimezone(timezone.utc) if start.tzinfo else start.replace(tzinfo=timezone.utc);e=end.astimezone(timezone.utc) if end.tzinfo else end.replace(tzinfo=timezone.utc);return [datetime(y,1,1,tzinfo=timezone.utc) for y in range(s.year+1,e.year+1)]
def main():
 ...
 with db_connect() as conn:
  ...
  cutoffs=build_cutoffs(matches[0].played_at,matches[-1].played_at);version='p0-shadow-'+started.strftime('%Y%m%d%H%M%S');cutoff=started;_lock(conn)
  try:
   with conn.transaction():
    ...
    run_id=conn.execute("insert into internal.prediction_training_runs(model_version_id,status,requested_cutoff,started_at,metrics) values (%s,'RUNNING',%s,%s,%s) returning id::text as id",(model_id,cutoff,started,json.dumps({'source':'s3_fixture_archive','settled_matches':len(matches),'feature_layer':'enabled'}))).fetchone()['id']
  ...
  try:
   ...
   for no,(train,test,fc) in enumerate(folds,1):
    if not train or not test:continue
    features=build_feature_index_for_matches(conn,test,fc);preds=run_fold(train,test,fc,features=features);score=_metrics(test,preds);total+=len(preds);tb+=score['brier_1x2']*len(preds);tl+=score['log_loss_1x2']*len(preds);tr+=score['rps_1x2']*len(preds);years.append(fc.year);coverage=sum(bool(features.get(m.match_id) and features[m.match_id].values) for m in test);s={'fold_no':no,'status':'SUCCEEDED','cutoff':fc.isoformat(),'train':len(train),'test':len(test),'predictions':len(preds),'feature_snapshots':len(features),'feature_covered_fixtures':coverage,'feature_coverage':coverage/len(test) if test else 0.,**score};summaries.append(s)
    with conn.transaction():
     for s in summaries:conn.execute("insert into internal.prediction_training_folds(training_run_id,fold_no,train_cutoff,test_start,test_end,status,metrics) values (%s,%s,%s,%s,%s,'SUCCEEDED',%s)",(run_id,s['fold_no'],s['cutoff'],min(m.played_at for m in build_walk_forward_folds(matches,[datetime.fromisoformat(s['cutoff'])])[0][1]),max(m.played_at for m in build_walk_forward_folds(matches,[datetime.fromisoformat(s['cutoff'])])[0][1]),json.dumps(s)))
    ...
    print(json.dumps({'status':'SUCCEEDED','training_run_id':run_id,'model_version_id':model_id,'version':version,**{k:metrics[k] for k in ('settled_matches','complete_oos_seasons','predictions','validation_status','validation_eligible','artifact_uri','artifact_sha256')}},sort_keys=True))
