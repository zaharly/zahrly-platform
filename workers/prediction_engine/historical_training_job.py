...
def _fit_dc_state(matches, cutoff, policy):
    ...
    attack = {
        team: (value / per_team_weight) / max(league_rate, 1e-12)
        for team, value in goals_for.items()
    }
    defense = {
        team: (value / per_team_weight) / max(league_rate, 1e-12)
        for team, value in goals_against.items()
    }
    ...
    return {
        ...
        'glicko': {
            ...
            'ratings': {
                team: {
                    'rating': state.rating,
                    'rd': state.rating_deviation,
                    'volatility': state.volatility,
                }
                for team, state in glicko_ratings.items()
            },
        },
        'dixon_coles': {
...
def main():
    ...
    with db_connect() as conn:
        ...
        if not cutoffs:
            raise RuntimeError('prediction_training_gate_failed:no_cutoffs')
        ...
        with conn.transaction():
            with conn.cursor() as cur:
                cur.execute(
                    "insert into public.model_versions(family,version,status,training_cutoff) values (%s,%s,'SHADOW',%s) returning id::text as id",
                    ('prediction_engine', version, cutoff),
                )
                model_id = cur.fetchone()['id']
                cur.execute(
                    "insert into internal.prediction_training_runs(model_version_id,status,requested_cutoff,started_at,metrics) values (%s,'RUNNING',%s,%s,%s) returning id::text as id",
                    (model_id, cutoff, started, json.dumps({'source': 's3_fixture_archive', 'settled_matches': len(matches)})),
                )
                run_id = cur.fetchone()['id']

        try:
            folds = build_walk_forward_folds(matches, cutoffs, test_window_days=30)
            summaries = []
            briers = []
            total_predictions = 0
            ...
            for fold_no, (train, test, fold_cutoff) in enumerate(folds, 1):
                if not train or not test:
                    ...
                    brier = sum(
                        (prediction.p_home - (match.home_goals > match.away_goals)) ** 2
                        + (prediction.p_draw - (match.home_goals == match.away_goals)) ** 2
                        + (prediction.p_away - (match.home_goals < match.away_goals)) ** 2
                    ...
                    with conn.cursor() as cur:
                        cur.execute(
                            ...
                            (
                                ...
                                fold_cutoff,
                                fold_cutoff,
                                fold_cutoff + timedelta(days=30),
                                summary['status'],
                                json.dumps(summary),
                            ),
                        )

            successful_folds = [summary for summary in summaries if summary['status'] == 'SUCCEEDED']
            if not successful_folds:
                raise RuntimeError('prediction_training_gate_failed:no_successful_folds')
            ...
            metrics = {
                ...
                'brier_1x2_mean': sum(briers) / len(briers),
            ...
            with conn.transaction():
                with conn.cursor() as cur:
                    cur.execute(
                        "update internal.prediction_training_runs set status='SUCCEEDED',finished_at=%s,metrics=%s where id=%s",
                        (datetime.now(timezone.utc), json.dumps(metrics), run_id),
                    )
                    cur.execute(
                        "update public.model_versions set artifact_uri=%s where id=%s",
                ...
                json.dumps(
                    {
                        ...
                        'successful_folds': len(successful_folds),
                        'predictions': total_predictions,
                        'brier_1x2_mean': metrics['brier_1x2_mean'],
            ...
            with conn.transaction():
                with conn.cursor() as cur:
                    cur.execute(
                        "update internal.prediction_training_runs set status='FAILED',finished_at=%s,metrics=coalesce(metrics,'{}'::jsonb)||%s::jsonb where id=%s",
                        (datetime.now(timezone.utc), json.dumps({'error': str(exc)}), run_id),
                    )
            raise
...
def build_cutoffs(start, end):
    ...
    while current < end:
        try:
            nxt = current.replace(year=current.year + 1)
        except ValueError:
            nxt = current.replace(year=current.year + 1, day=28)
        out.append(nxt if nxt < end else end)
        current = nxt
    return [cutoff for cutoff in out if cutoff > start]
