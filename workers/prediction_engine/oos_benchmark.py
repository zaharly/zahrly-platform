...
def db_connect():
    ...
    # Historical OOS is an isolated batch workload. Do not let the server's
    # default statement_timeout interrupt a large index maintenance operation.
...
def main(training_run_id: str|None=None):
    with db_connect() as conn:
        ...
        for fold_db,(train,test,cutoff) in zip(folds,wf):
            ...
            conn.commit()
            preds=run_fold(train,test,cutoff); emp=empirical_probs(train); fold_model=[]; fold_emp=[]; fold_market=[]
            buffer=[]
            for m,pred in zip(test,preds):
                ...
                model_rows.append({'confidence':ms[3],'correct':ms[4],'score':ms}); emp_rows.append({'confidence':ep[3],'correct':ep[4],'score':ep}); fold_model.append(ms); fold_emp.append(ep)
                if kp: market_rows.append({'confidence':kp[3],'correct':kp[4],'score':kp}); fold_market.append(kp)
                clv=None
                ...
                if len(buffer)>=BATCH_SIZE:
                    with conn.cursor() as cur:
                        cur.executemany("insert into internal.prediction_oos_benchmark(training_run_id,model_version_id,fixture_id,fold_no,played_at,outcome,model_p_home,model_p_draw,model_p_away,empirical_p_home,empirical_p_draw,empirical_p_away,market_p_home,market_p_draw,market_p_away,market_snapshot_at,metrics) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",buffer)
                    conn.commit(); buffer=[]
            if buffer:
                with conn.cursor() as cur:
                    cur.executemany("insert into internal.prediction_oos_benchmark(training_run_id,model_version_id,fixture_id,fold_no,played_at,outcome,model_p_home,model_p_draw,model_p_away,empirical_p_home,empirical_p_draw,empirical_p_away,market_p_home,market_p_draw,market_p_away,market_snapshot_at,metrics) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",buffer)
            ...
            with conn.cursor() as cur:
                rows=[(eval_run,eval_fold,'ALL','Brier',fm['brier'],n,json.dumps({'model':'prediction_engine'})),(eval_run,eval_fold,'ALL','LogLoss',fm['log_loss'],n,'{}'),(eval_run,eval_fold,'ALL','RPS',fm['rps'],n,'{}'),(eval_run,eval_fold,'ALL','ECE',fm['ece'],n,'{}')]
                cur.executemany("insert into internal.evaluation_metrics(run_id,fold_id,segment,metric_name,metric_value,sample_count,metadata) values (%s,%s,%s,%s,%s,%s,%s)",rows)
        ...
        summary={'oos_n':n,'model':model,'empirical_baseline':empirical,'market':market,'clv':clv,'history_gate':{'min_oos_predictions':MIN_OOS_PREDICTIONS,'min_complete_oos_seasons':MIN_COMPLETE_SEASONS,'oos_predictions_pass':n>=MIN_OOS_PREDICTIONS,'oos_seasons_pass':len([f for f in folds if f['status']=='SUCCEEDED'])>=MIN_COMPLETE_SEASONS},'evaluation_run_id':eval_run}
