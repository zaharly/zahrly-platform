...
# Neutral, deterministic priors are used only for teams absent from the trained
# artifact. They prevent a newly promoted/newly observed team from blocking the
# rolling engine while keeping the prediction conservative and auditable.
COLD_START_RATING = 1500.0
COLD_START_ATTACK = 1.0
COLD_START_DEFENSE = 1.0
...
def _team_params(artifact: dict[str, Any], team_id: str) -> tuple[float, float, float, str]:
    ...
    version = cold.get("cold_start_policy_version")
    ...
    # Artifact-level neutral fallback. This is deliberately not a historical
    # team state: it represents league-neutral information for an unseen team.
    # The source is recorded so gates/read models remain transparent.
    rate = float(dc.get("league_rate", 0.0) or 0.0)
    if 0.25 <= rate <= 4.0:
        return COLD_START_RATING, COLD_START_ATTACK, COLD_START_DEFENSE, "COLD_START_NEUTRAL"
...
def score_matrix(home_lambda: float, away_lambda: float, rho: float, max_goals: int) -> list[list[float]]:
    if home_lambda <= 0 or away_lambda <= 0:
        raise RuntimeError("invalid_lambda")
    matrix = []
    total = 0.0
    for x in range(max_goals + 1):
        ...
        matrix.append(row)
    if total <= 0:
        raise RuntimeError("invalid_score_state")
...
def calibrate_1x2(matrix: list[list[float]], temperature: float) -> dict[str, float]:
    ...
    if not math.isfinite(temperature) or not 0.60 <= temperature <= 1.00:
        raise RuntimeError("invalid_calibration_temperature")
    z = [math.log(max(1e-15, v)) / temperature for v in values]
    m = max(z)
    w = [math.exp(v - m) for v in z]
    s = sum(w)
...
def predict_fixture(fixture: dict[str, Any], artifact: dict[str, Any], temperature: float) -> dict[str, Any]:
    ...
    rate = float(dc["league_rate"])
    rho = float(dc.get("rho", 0.0))
    home_adv = float(dc.get("home_advantage", 0.0))
    max_goals = min(max(int(dc.get("max_goals", 10)), 6), 12)
    if not 0.25 <= rate <= 4.0:
        raise RuntimeError("league_rate_out_of_bounds")
    elo_weight = float(dc.get("elo_context_weight", 0.0))
    scale = max(float(elo.get("rating_scale", 400.0)), 1.0)
    rating_adv = float(elo.get("home_advantage", 0.0))
    ...
    matrix = score_matrix(lh, la, rho, max_goals)
    probabilities = calibrate_1x2(matrix, temperature)
    return {
        "lambdas": {"home": lh, "away": la},
        "raw": calibrate_1x2(matrix, 1.0),
        "probabilities": probabilities,
        "score_matrix": matrix,
        "state_sources": {"home": home_source, "away": away_source},
    }
