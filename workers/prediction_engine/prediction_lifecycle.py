...
from workers.prediction_engine.prediction_model_core import build_benchmarks, calibration, db_connect, load_artifact, load_model, predict_fixture, provider_key, settle, sha256_json
...
def valid_probabilities(p:dict[str,Any])->bool:
    ...
    return all(0.0<=x<=1.0 for x in values) and abs(sum(values)-1.0)<=1e-6


def state_coverage(artifact:dict[str,Any],fixture:dict[str,Any])->tuple[float,list[str],dict[str,str]]:
    from workers.prediction_engine.prediction_model_core import _required_team_params
    checks=[]; missing=[]; sources={}
    for side,team_id in (("home",str(fixture["home_team_id"])),("away",str(fixture["away_team_id"]))):
        try:
            _,_,_,source=_required_team_params(artifact,team_id); checks.append(True); sources[side]=source
...
def market_snapshot(conn,fixture_id,kickoff,as_of):
    ...
    for book in books.values():
        ...
        closing={k:max(book[k],key=lambda x:x[1]) for k in ("H","D","A")}; inv=[1.0/closing[k][0] for k in ("H","D","A")]; total=sum(inv)
        if total<=0:continue
...
def append_evidence(conn,base,model_id,evidence_type,probability,snapshot,artifact_sha):
    ...
    conn.execute("insert into public.prediction_evidence_updates(baseline_id,evidence_seq,evidence_type,current_probability,model_version_id,evidence_snapshot_hash) values(%s::uuid,%s,%s,%s,%s::uuid,%s)",(base["id"],seq,evidence_type,probability,model_id,h)); return seq,h,True


def persist_market_states(conn,baseline_id,episode_id,markets_map,status):
    rows=[]
    for key,item in markets_map.items():
        p=float(item["probability"])
        if 0<=p<=1: rows.append((baseline_id,episode_id,key,p,1.0/max(1e-12,p),status))
    with conn.cursor() as cur:
...
def record_gate(conn,fixture,episode,model,release,artifact,artifact_sha,cal_status,training_status,probabilities,now,snapshot):
    kickoff=utc(fixture["kickoff_at"]); hours=(kickoff-now).total_seconds()/3600.0; identity=1.0 if fixture.get("home_team_id") and fixture.get("away_team_id") and episode.get("id") else 0.0
    coverage,missing,sources=state_coverage(artifact,fixture); health,health_errors=model_health(release,model,artifact,cal_status,training_status,artifact_sha)
    cutoff=parse_cutoff(model.get("training_cutoff") or artifact.get("training_cutoff") or (artifact.get("metadata") or {}).get("training_cutoff")); no_leak=bool(cutoff and cutoff<kickoff and cutoff<=now)
    if snapshot and snapshot.get("snapshot_at"):
        s=utc(snapshot["snapshot_at"]); no_leak=no_leak and s<=now and s<kickoff
    canonical=str(fixture.get("status","")).lower()=="scheduled" and str(episode.get("episode_status","")).upper()=="ACTIVE" and kickoff>now; prob_ok=valid_probabilities(probabilities)
    eligible=(0<=hours<=T7_DAYS*24 and canonical and identity>=1.0 and coverage>=1.0 and health=="HEALTHY" and no_leak and prob_ok)
...
def baseline(conn,episode_id):
    return conn.execute("select id::text as id,model_version_id::text as model_version_id,policy_bundle_id::text as policy_bundle_id,baseline_pick,baseline_probability,baseline_hash from public.prediction_baselines where episode_id=%s::uuid limit 1",(episode_id,)).fetchone()


def publish_read_model(conn,fixture,episode,base,prediction,markets_map,gate,seq,evidence_hash,market,now):
    row=conn.execute("select version from public.prediction_read_models where fixture_id=%s::uuid and episode_id=%s::uuid limit 1",(fixture["id"],episode["id"])).fetchone(); version=int(row["version"] or 0)+1 if row else 1
    p=prediction["probabilities"]; top=max((("HOME",p["home"]),("DRAW",p["draw"]),("AWAY",p["away"])),key=lambda x:x[1]); kickoff=utc(fixture["kickoff_at"])
    payload={"schema_version":"prediction-read-model-v2","publication_status":"SHADOW_PUBLISHED","fixture_id":fixture["id"],"episode_id":episode["id"],"kickoff_at":kickoff.isoformat(),"locked":kickoff<=now,"locked_at":kickoff.isoformat() if kickoff<=now else None,"baseline":{"id":base["id"],"pick":base["baseline_pick"],"probability":float(base["baseline_probability"]),"hash":base["baseline_hash"]},"assessment":{"pick":top[0],"probability":float(top[1]),"probabilities":p,"fair_odds":{k:1.0/max(1e-12,float(v)) for k,v in p.items()}},"markets":markets_map,"evidence":{"sequence":seq,"snapshot_hash":evidence_hash},"gate":gate,"market_snapshot":market,"model_version_id":prediction["model_version_id"],"artifact_sha256":prediction["artifact_sha256"],"calibration_version":prediction["calibration_version"],"published_at":now.isoformat()}
...
def process(conn,fixture,episode,release,model,pol,artifact,artifact_sha,cal_version,cal_status,training_status,temperature,now):
    ...
    markets_map=derived_markets(prediction,artifact); market_hash=persist_market_evidence(conn,fixture["id"],episode["id"],model["id"],snapshot) if snapshot else None
    persist_market_states(conn,base["id"],episode["id"],markets_map,"SHADOW")
    same_key={"HOME":"home","DRAW":"draw","AWAY":"away"}[base["baseline_pick"]]; same_pick_probability=float(prediction["probabilities"].get(same_key,base["baseline_probability"]))
    etype="BASELINE_CREATED" if created else ("MARKET_EVIDENCE_UPDATE" if snapshot else "RUNTIME_REASSESSMENT"); seq,eh,new_evidence=append_evidence(conn,base,model["id"],etype,same_pick_probability,snapshot,artifact_sha)
    pred={**prediction,"model_version_id":model["id"],"artifact_sha256":artifact_sha,"calibration_version":cal_version}; market={"probabilities":snapshot["probabilities"],"bookmaker_count":snapshot["bookmaker_count"],"snapshot_at":snapshot["snapshot_at"]} if snapshot else None
    rm=publish_read_model(conn,fixture,episode,base,pred,markets_map,gate,seq,eh,market,now)
    return {"fixture_id":fixture["id"],"status":"PUBLISHED_SHADOW" if gate["eligible"] else "BASELINE_EXISTS","gate":gate,"baseline_id":base["id"],"baseline_created":created,"evidence_created":new_evidence,"evidence_seq":seq,"read_model_published":True,"read_model_version":rm,"market_count":len(markets_map),"corners_supported":any(k.startswith("corners_") for k in markets_map),"cards_supported":any(k.startswith("cards_") for k in markets_map),"market_snapshot_hash":market_hash}


def main():
    now=datetime.now(timezone.utc)
    with db_connect() as conn:
        release,loaded_model=load_model(conn); model=dict(loaded_model); artifact,artifact_sha=load_artifact(model["artifact_uri"]); temperature,cal_version,cal_status=calibration(conn); training=training_state(conn,model["id"]); pol=policy(conn); markets(conn)
