...
def main() -> None:
    ...
    with psycopg.connect(db_url, row_factory=dict_row, connect_timeout=20, sslmode='require') as conn:
        ...
        for checkpoint_time, elo_state, glicko_state in checkpoints:
            for team, state in elo_state.items():
                ...
                if not team_id:
                    unmapped += 1
                    continue
                rows.append((run['model_version_id'], 'elo-v1', team_id, state.rating, state.rating_deviation, state.volatility, checkpoint_time))
            for team, state in glicko_state.items():
                team_id = canonical_team_id(team)
                if not team_id:
                    unmapped += 1
                    continue
                rows.append((run['model_version_id'], 'glicko-v1', team_id, state.rating, state.rating_deviation, state.volatility, checkpoint_time))
        ...
        with conn.transaction():
            conn.execute(
                "insert into public.policy_versions(policy_type,version,payload) values (%s,%s,%s) "
        ...
        print(json.dumps({
            ...
            'persisted_checkpoint_rows': len(rows),
