...
def main() -> None:
    ...
    with psycopg.connect(db_url, row_factory=dict_row, connect_timeout=20, sslmode='require') as conn:
        ...
        if run_id:
            run = conn.execute("select id::text as id,model_version_id::text as model_version_id from internal.prediction_training_runs where id=%s and status='SUCCEEDED'", (run_id,)).fetchone()
        else:
        ...
        known_team_ids = {str(r['id']) for r in conn.execute("select id from public.teams").fetchall()}
        external_to_canonical = {str(r['external_team_id']): str(r['team_id']) for r in conn.execute("select external_team_id,team_id from public.team_aliases where provider='api-football' and external_team_id is not null and team_id is not null").fetchall()}
