...
def _team_rates(train,cutoff,policy):
    cutoff=_utc(cutoff);gf={};ga={};team_w={};tw=tg=0.
    for m in train:
        ...
        for t,x,y in ((m.home_team_id,m.home_goals,m.away_goals),(m.away_team_id,m.away_goals,m.home_goals)):
            gf[t]=gf.get(t,0.0)+w*x;ga[t]=ga.get(t,0.0)+w*y;team_w[t]=team_w.get(t,0.0)+w
        tw+=2*w;tg+=w*(m.home_goals+m.away_goals)
    rate=max(tg/max(tw,1e-12),1e-6);attack={};defense={}
    for team,w in team_w.items():
        raw_attack=(gf.get(team,0.0)/max(w,1e-12))/rate;raw_defense=(ga.get(team,0.0)/max(w,1e-12))/rate;shrink=w/(w+RATE_PRIOR_WEIGHT)
        attack[team]=min(RATE_CEILING,max(RATE_FLOOR,1.0+shrink*(raw_attack-1.0)));defense[team]=min(RATE_CEILING,max(RATE_FLOOR,1.0+shrink*(raw_defense-1.0)))
...
def predict_with_state(match,ratings,train,cutoff,elo_policy=EloPolicy(),dc_policy=DixonColesPolicy(),features=None,team_rates=None):
    ...
    attack,defense,rate=team_rates;hr=ratings.get(match.home_team_id,EloState(elo_policy.initial_rating));ar=ratings.get(match.away_team_id,EloState(elo_policy.initial_rating));ef=1/(1+exp(-((hr.rating+elo_policy.home_advantage)-ar.rating)/elo_policy.rating_scale*2.302585092994046));ha=attack.get(match.home_team_id,1);aa=attack.get(match.away_team_id,1);hd=defense.get(match.home_team_id,1);ad=defense.get(match.away_team_id,1);hl=max(.05,rate*exp(dc_policy.home_advantage)*ha/max(ad,.25)*(.75+.5*ef)*_feature_factor(match,features,'home'));al=max(.05,rate*aa/max(hd,.25)*(1.25-.5*ef)*_feature_factor(match,features,'away'));ph,pd,pa=result_probabilities(probability_matrix(hl,al,dc_policy.rho,dc_policy.max_goals));return Prediction(match.match_id,match.home_team_id,match.away_team_id,ph,pd,pa,hl,al)
...
def build_walk_forward_folds(matches:Iterable[Match],cutoffs:Sequence[datetime],test_window_days:int=365):
    ...
    if len(season_labels)>=2:
        ...
        for target_index in range(1,len(season_labels)):
            target=season_labels[target_index];target_matches=[m for m,label in labelled if label==target]
            if not target_matches:continue
            cutoff=min(_utc(m.played_at) for m in target_matches);train=[m for m,label in labelled if label is not None and season_index[label]<target_index and _utc(m.played_at)<cutoff];test=[m for m,label in labelled if label==target and _utc(m.played_at)>=cutoff]
            if not train or not test:continue
            train_seasons={normalize_season_label(m.season) for m in train if normalize_season_label(m.season) is not None};test_seasons={normalize_season_label(m.season) for m in test if normalize_season_label(m.season) is not None};overlap=train_seasons & test_seasons
