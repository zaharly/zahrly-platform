...
def _team_rates(train,cutoff,policy):
    cutoff=_utc(cutoff); gf={};ga={};tw=tg=0.0
    for m in train:
    ...
    rate=max(tg/max(tw,1e-12),1e-6); den=max(tw/2,1e-12)
    return {t:(v/den)/rate for t,v in gf.items()},{t:(v/den)/rate for t,v in ga.items()},rate

def _feature_factor(match,features,side):
    if not features:return 1.0
    s=features.get(match.match_id)
    if not s:return 1.0
    x=s.get(f'{side}.last5.fixture_statistics.xg.mean')
    shots=s.get(f'{side}.last5.fixture_statistics.shots_on_target.mean')
    factor=1.0
    if x is not None:factor*=min(1.15,max(.85,1.0+0.12*(x-1.0)))
...
def build_walk_forward_folds(matches:Iterable[Match],cutoffs:Sequence[datetime],test_window_days:int=365):
    ...
    for raw in cutoffs:
        cutoff=_utc(raw)
        if seasoned:
            y=cutoff.year;train=[m for m in ordered if m.season<y];test=[m for m in ordered if m.season==y]
        else:
